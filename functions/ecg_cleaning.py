...
def find_r_peaks_based_on_ext_ecg(self, full_data, times):
    ...
    return lfp_peak_indices, polarity
...
def clean_ecg_interpolation(self):
    ...
    if self.dataset_extra.selected_channel_name_ecg is not None:
        final_peaks, polarity = find_r_peaks_based_on_ext_ecg(self, full_data, times)
    ...
    ############################################################################
        ...
        def after_dialog(start, end, threshold, polarity, full_data, times):
            ...
            final_peaks, mean_epoch = find_r_peaks_in_lfp_channel(self, data_crop, polarity, threshold, window=[-0.5, 0.5])
...
def clean_ecg_template_sub(self):
    ...
    ############################################################################
        ...
        def after_dialog(start, end, threshold, polarity, full_data, times):
            ...
            final_peaks, mean_epoch = find_r_peaks_in_lfp_channel(self, data_crop, polarity, threshold, window = [-0.2, 0.2])
    ...
    print(f'len-final_peaks): {len(final_peaks)}')
